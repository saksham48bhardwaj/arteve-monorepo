...
// This is the musician app. Accounts are single-role (one Supabase project is
// shared by both apps), so an organizer account must not be allowed to operate
// here. If a mismatched account is detected we sign it out and bounce to login
// with a clear pointer to the correct app.
const APP_ROLE = 'musician';
const OTHER_APP = { label: 'organizer', url: 'https://organizer.arteve.in' };

// Pages where the guard must stay out of the way (auth + onboarding flows).
  ...
  useEffect(() => {
    ...
    async function check(uid: string) {
      ...
      if (prof?.role && prof.role !== APP_ROLE) {
        toast.error(`This is an ${prof.role} account. Please use the ${OTHER_APP.label} app at ${OTHER_APP.url.replace('https://', '')}.`);
