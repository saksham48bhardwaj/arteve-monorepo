...
export default function NewBitPage() {
  ...
  function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f && !f.type.startsWith('video')) {
      setErr('Please select a video file'); return;
    }
    setErr(null);
    setFile(f);
  ...
  return (
    <main className="mx-auto max-w-xl p-6 space-y-4">
      <h1 className="text-2xl font-semibold">New Bit</h1>

      <label className="block">
        <span className="text-sm">Short video</span>
        <input type="file" accept="video/*" onChange={onFile} className="mt-1" />
      </label>

      {preview && (
        <video className="w-full rounded-2xl border" src={preview} controls onLoadedMetadata={onLoadedMetadata} />
      )}
